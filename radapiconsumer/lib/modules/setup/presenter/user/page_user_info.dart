      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.hasData) {
          ...
          if (state is StartState) {
            ...
            String email = SetupUtil().setupEmail();
            bool emailExists = list.contains(email);
            // Se e-mail não existe no resultado,
            // adiciona o mesmo via API
            if (!emailExists) {
              StaffModel me = StaffModel(
                  email: email,
                  password: 'first1283ouoiu837121',
            ...
            return Expanded(
                child: ListView.builder(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      debugPrint('f5025 - item: $index');
                      debugPrint(
                          'f5025 - compare: my email => [$email] x result email => [${item.email}]');
                      if (item.email == null) {
                        return Container();
